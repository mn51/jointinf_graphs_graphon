...
# sparse_stat_mod3
@jit(nopython=True)
def in_wrapper_sparse_stat_mod3(zeta, X,
    ...
    z_st=[]
    for k in range(K):
        z_st.append(np.arange(int(N[k])))
        for z_idx in range(int(N[k])):
            z_st[k][z_idx] = int(zeta[k][z_idx]*G)
        z_st[k][z_st[k]>=G]=G-1
    z_orig=z_st.copy()

    I_z_st = [np.eye(G)[z_st[k],:] for k in range(K)]
    Sigma_L = [(np.kron(I_z_st[k],I_z_st[k])[:,L_ind_W] + np.kron(I_z_st[k],I_z_st[k])[:,U_ind_W])[L_ind_list[k]] for k in range(K)]
    Sigma_D = [(np.kron(I_z_st[k],I_z_st[k])[:,D_ind_W])[L_ind_list[k]] for k in range(K)]
    Sigma = np.zeros((np.sum(L),G+J))
    for k in range(K):
        Sigma[np.sum(L[:k]):np.sum(L[:k])+L[k],:G] = Sigma_D[k]
        Sigma[np.sum(L[:k]):np.sum(L[:k])+L[k],G:] = Sigma_L[k]
    # i_k = [(np.kron(I_z_st[k],I_z_st[k])[:,L_ind_W] + np.kron(I_z_st[k],I_z_st[k])[:,U_ind_W])[L_ind_list[k]] for k in range(K)]
    # Sigma = np.zeros((np.sum(L),J))
    # for k in range(K):
    #     Sigma[np.sum(L[:k]):np.sum(L[:k])+L[k],:] = i_k[k]
    ...
    MAX_ETA = np.min([4,int(np.min(N)/3)])
    Q = np.zeros((K,np.sum(L)))
    for k in range(K):
        Q[k,np.sum(L[:k]):np.sum(L[:k])+L[k]]=1
    ...
    zeta_st = [z_st[k]/(G-1) for k in range(K)]
    zeta_orig = zeta_st.copy()
    u1 = np.zeros(np.sum(L))
    u2 = np.zeros(K)
    u3 = np.zeros(G+J)
    ...
    Phi_w1 = 4*lmbda*d2.T@d2 + 4*lmbda*d1.T@d1 + Lmbda*Sigma.T@Sigma + rho3*np.eye(G+J)
    ...
    for admm_iter in range(num_admm_iters):
        ...
        T_st = [lowtri2mat(t_st[k]) for k in range(K)]

        # w-update
        Phi_w2 = Lmbda*Sigma.T@t_st + rho3*(v_st - u3)
        # w_st = Phi_w1@Phi_w2
        w_st = sp.linalg.cho_solve(sp.linalg.cho_factor(Phi_w1),Phi_w2)

        W_st = np.zeros((G,G))
        W_st[np.eye(G)==1] = w_st[:G]
        W_st += lowtri2mat(w_st[G:])
        ...
        for k in range(K):
            eta = np.zeros(N[k])
            for i in range(N[k]):
                ETA_RANGE = np.arange(np.max([-z_orig[k][i],-MAX_ETA]),np.min([G-1-z_orig[k][i],MAX_ETA])+1)
                eta[i] = ETA_RANGE[np.argmin([np.linalg.norm(T_st[k] - \
                              np.eye(G)[(z_orig[k] + eta_i*np.eye(N[k])[i,:]).astype(int),:]@\
                              W_st@\
                              np.eye(G)[:,(z_orig[k] + eta_i*np.eye(N[k])[i,:]).astype(int)]\
                              )**2 for eta_i in ETA_RANGE])]
                z_st[k][i]=z_orig[k][i]+eta[i]
        I_z_st = [np.eye(G)[z_st[k],:] for k in range(K)]
        Sigma_L = [(np.kron(I_z_st[k],I_z_st[k])[:,L_ind_W] + np.kron(I_z_st[k],I_z_st[k])[:,U_ind_W])[L_ind_list[k]] for k in range(K)]
        Sigma_D = [(np.kron(I_z_st[k],I_z_st[k])[:,D_ind_W])[L_ind_list[k]] for k in range(K)]
        Sigma = np.zeros((np.sum(L),G+J))
    ...
    return s_ret, t_ret, v_st, z_st

def sparse_stat_mod3(zeta_X,
                     theta=10,rho1=10,rho2=10,rho3=10,phi=10,eta=1e-1,lmbda=1,Lmbda=10,
                     eps_abs=0.,eps_rel=1e-3,
                     num_bins=5,pg_step_size=.01,pg_stop=1e-2,
                     num_admm_iters=1000,num_subadmm_iters=200,num_pg_iters=200,
    ...
    s_est, t_est, w_est, z_est = in_wrapper_sparse_stat_mod3(zeta_NUMBA,X_NUMBA,
    ...
    for k in range(K):
        s_est[k] = s_est[k].astype(int)
    return s_est, t_est, w_est, z_est
