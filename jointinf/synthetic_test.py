...
Generate graphs from a given graphon, then generate graph signals from sampled graphs.
Signals can be stationary, smooth, or MRF signals.

Node set can be same, different with same sizes, or have different sizes.

Choose different graphons, including ER, SBM, quad, similar, etc.
'''
...
for nodeset in ['diffsize']:
    ...
    #-------------------------
    signal_range = (np.logspace(2,5,7)).astype(int)
    ...
    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    '''
    Given a graph signal set and graphon sample points zeta,
    estimate graphs given different objectives with and without proposed framework.
    '''
    #-------------------------------------------------------------------------------
    # Estimate graphs
    ...
    for graph_trial in range(num_graph_trials):
        ...
        for num_sig_trial in range(len_signal_range):
            ...
            for method_iter in range(num_methods):
                if method_vec[method_iter]:
                    ...
                    if method_iter==0:
                        s_curr = sparse_stat(X_curr)
                    elif method_iter==1:
                        s_curr,t_curr = stat_mod1(X_curr)
                    elif method_iter==2:
                        s_curr,t_curr,w_curr = stat_mod2(zeta_X_curr)
                    elif method_iter==3:
                        s_curr,t_curr = sparse_stat_mod1(X_curr)
                    elif method_iter==4:
                        s_curr,t_curr,w_curr = sparse_stat_mod2(zeta_X_curr)
                    elif method_iter==5:
                        s_curr = pairdiff_stat(X_curr)
                    elif method_iter==6:
                        s_curr,t_curr = pairdiff_stat_mod1(X_curr)
                    elif method_iter==7:
                        s_curr,t_curr,w_curr = pairdiff_stat_mod2(zeta_X_curr)
                    else:
                    ...
                    s_ERROR[method_iter][graph_trial,num_sig_trial] = np.sum([np.linalg.norm(s_curr[k]-s_TRUE[graph_trial][k],1) for k in range(K)])/np.sum(L)
        ...
        for method_iter in range(num_methods):
            if method_vec[method_iter]:
                print(f'  Mean error s: {np.mean(s_ERROR[method_iter][:,num_sig_trial]):.4f} ('+method_names[method_iter]+')')
                exp_status_file = open(status_file_name,'a')
                exp_status_file.write(f'  Mean error s: {np.mean(s_ERROR[method_iter][:,num_sig_trial]):.4f} ('+method_names[method_iter]+')' + '\n')
                exp_status_file.close()

                if any(method_iter==np.array([1,2,3,4,6,7])):
                    print(f'  Mean error t: {np.mean(t_ERROR[method_iter][:,num_sig_trial]):.4f} ('+method_names[method_iter]+')')
                    exp_status_file = open(status_file_name,'a')
                    exp_status_file.write(f'  Mean error t: {np.mean(t_ERROR[method_iter][:,num_sig_trial]):.4f} ('+method_names[method_iter]+')' + '\n')
                    exp_status_file.close()

                    if any(method_iter==np.array([2,4,7])):
                        print(f'  Mean error w: {np.mean(w_ERROR[method_iter][:,num_sig_trial]):.4f} ('+method_names[method_iter]+')')
                        exp_status_file = open(status_file_name,'a')
                        exp_status_file.write(f'  Mean error w: {np.mean(w_ERROR[method_iter][:,num_sig_trial]):.4f} ('+method_names[method_iter]+')' + '\n')
                        exp_status_file.close()
